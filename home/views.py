...
def stock_guidance(request):
    return render(request, 'stock_guide.html')
...
def adminloginPage(request):
    if request.method == "POST":
        ...
        if user_email and user_password:
            # try:
            #     user = Admin_model.objects.get(email='lalith@gmail.com', password='lalith')
            #     request.session['Email'] = user.email
            #     request.session['Name'] = user.name
...
def model_prediction(stock):
    import numpy as np
    input_path = '/home/lalith/Documents/Projects/Ramesh_Project/Stock-Trading/stock_trading/home/model_feed.csv'
